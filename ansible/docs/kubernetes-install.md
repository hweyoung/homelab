...
playbooks/bootstrap.yml
  ...
  └── playbooks/kubernetes/node-config.yml → k8s_node_config
```
...
```bash
./scripts/run-playbook.sh kubespray-force playbooks/bootstrap.yml \
  --tags kubespray \
...
`make argocd`는 ArgoCD release가 없을 때만 최초 설치한다. 기존 release가 있으면 chart와
values를 변경하지 않고 bootstrap Secret과 root Application만 멱등적으로 확인한다.
chart 또는 values 변경은 [argocd-upgrade.md](argocd-upgrade.md)의 승인된 upgrade workflow를
사용한다.
